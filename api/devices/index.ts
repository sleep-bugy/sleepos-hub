...
export default async function handler(request: Request) {
  ...
  if (method === 'GET') {
    // Return all devices - in production this would connect to database
    return new Response(JSON.stringify(mockDevices), {
      status: 200,
      headers: { 'Content-Type': 'application/json' }
    });
  } else if (method === 'POST') {
    // Add a new device - in production this would connect to database
    const newDeviceData = await request.json();
    const newDevice = {
      ...newDeviceData,
      id: Math.max(...mockDevices.map(d => d.id), 0) + 1, // Generate new ID
      roms: [],
