// api/roms/index.ts - Handle all ROMs data
export const config = {
  runtime: 'edge',
};

// Store for demo purposes - this will be replaced with database queries in production
let mockDevices = [
  {
    id: 1,
    name: "Xiaomi Mi 9",
...
export default async function handler(request: Request) {
  ...
  if (method === 'GET') {
    // Get all ROMs from all devices
    const allRoms = [];
    for (const device of mockDevices) {
      if (Array.isArray(device.roms)) {
        allRoms.push(...device.roms.map(rom => ({...rom, deviceId: device.id})));
      }
    }
    ...
    // POST is not supported directly at /api/roms, redirect to device endpoint
    return new Response(JSON.stringify({ error: 'POST not supported here. Use /api/devices/{codename}/roms instead.' }), {
      status: 405,
      headers: { 'Content-Type': 'application/json' }
    });
  } else if (method === 'PUT' || method === 'DELETE') {
    // PUT and DELETE are not supported directly at /api/roms
    return new Response(JSON.stringify({ error: 'PUT and DELETE not supported here. Use specific device endpoints.' }), {
      status: 405,
      headers: { 'Content-Type': 'application/json' }
    });
  }
